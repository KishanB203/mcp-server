...
function analyze(diff, changedFiles) {
  ...
  if (addedLines > 1200) {
    warnings.push(`⚠️ Large change set: ${addedLines} lines added — consider splitting`);
  }
  ...
  if (consoleCount > 0) {
    warnings.push(`⚠️ console.log present: ${consoleCount} occurrence(s) — remove before merge`);
  } else {
  ...
  if (srcFiles.length > 0 && testFiles.length === 0) {
    warnings.push("⚠️ No test files detected in change set — add tests before merge");
...
export class PRReviewer {
  /**
   * @param {{ githubClient?: import("../infrastructure/github/github-client.js").GitHubClient }} [options]
   */
  constructor(options = {}) {
  ...
  async reviewPR({ repo, prNumber, baseBranch = "main", headBranch }) {
    ...
    // Load the four project rules files (silently tolerate missing files).
    const rules = [
      "coding-standard.md",
      "architecture.md",
      "naming-rule.md",
      ...
      .map((f) => readFile(path.join(rulesDir, f)))
      .filter(Boolean)
      .join("\n\n");
